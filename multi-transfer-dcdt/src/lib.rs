...
use dharitri_sc::{imports::*, storage::StorageKey};
    ...
    tx_batch_module::TxBatchModule + max_bridged_amount_module::MaxBridgedAmountModule
{
    ...
    #[only_owner]
    #[endpoint(batchTransferDcdtToken)]
    fn batch_transfer_dcdt_token(
        &self,
        batch_id: u64,
        ...
        let safe_address = self.dcdt_safe_contract_address().get();
        ...
        for eth_tx in transfers {
            ...
            valid_payments_list.push(DcdtTokenPayment::new(eth_tx.token_id, 0, eth_tx.amount));
        }
            ...
            OptionalValue::Some(current_batch) => {
                ...
                for tx_fields in all_tx_fields {
                    ...
                    if self.is_refund_valid(&token_identifier) {
                        ...
                        refund_payments.push(DcdtTokenPayment::new(token_identifier, 0, amount));
                    } else {
                ...
                let dcdt_safe_addr = self.dcdt_safe_contract_address().get();
                self.tx()
                    .to(dcdt_safe_addr)
                    .typed(dcdt_safe_proxy::DcdtSafeProxy)
    ...
    #[only_owner]
    #[endpoint(setDcdtSafeContractAddress)]
    fn set_dcdt_safe_contract_address(&self, opt_new_address: OptionalValue<ManagedAddress>) {
        match opt_new_address {
            OptionalValue::Some(sc_addr) => {
    ...
    fn is_refund_valid(&self, token_id: &TokenIdentifier) -> bool {
        ...
        if self.is_account_same_shard_frozen(sc_shard, &dcdt_safe_addr, token_id) {
            return false;
        }
        return true;
    }
    ...
    fn is_account_same_shard_frozen(
        ...
        let token_data = self
            .blockchain()
            .get_dcdt_token_data(dest_address, token_id, 0);
        token_data.frozen
    }
        ...
        for (eth_tx, p) in transfers.iter().zip(payments.iter()) {
            if self.blockchain().is_smart_contract(&eth_tx.to) {
                self.tx()
                    ...
                    .single_dcdt(&p.token_identifier, 0, &p.amount)
                    .sync_call();
            } else {
                self.tx()
                    .to(&eth_tx.to)
