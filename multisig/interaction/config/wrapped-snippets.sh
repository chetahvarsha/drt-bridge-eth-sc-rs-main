...
deployBridgedTokensWrapper() {
    ...
    drtpy contract deploy --bytecode=${BRIDGED_TOKENS_WRAPPER_WASM} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=60000000 \
    --send --outfile="deploy-bridged-tokens-wrapper-testnet.interaction.json" --proxy=${PROXY} --chain=${CHAIN_ID} || return

    TRANSACTION=$(drtpy data parse --file="./deploy-bridged-tokens-wrapper-testnet.interaction.json" --expression="data['emittedTransactionHash']")
    ADDRESS=$(drtpy data parse --file="./deploy-bridged-tokens-wrapper-testnet.interaction.json" --expression="data['contractAddress']")

    drtpy data store --key=address-testnet-bridged-tokens-wrapper --value=${ADDRESS}
...
setLocalRolesBridgedTokensWrapper() {
    CHECK_VARIABLES DCDT_SYSTEM_SC_ADDRESS UNIVERSAL_TOKEN BRIDGED_TOKENS_WRAPPER
    
    drtpy contract call ${DCDT_SYSTEM_SC_ADDRESS} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=60000000 --function="setSpecialRole" \
    --arguments str:${UNIVERSAL_TOKEN} ${BRIDGED_TOKENS_WRAPPER} str:DCDTRoleLocalMint str:DCDTRoleLocalBurn\
...
wrapper-pauseV2() {
    ...
    drtpy contract call ${BRIDGED_TOKENS_WRAPPER_v2} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=5000000 --function="pause" \
    --send --proxy=${PROXY} --chain=${CHAIN_ID} || return
}
...
wrapper-upgrade() {
    ...
    drtpy contract upgrade ${BRIDGED_TOKENS_WRAPPER} --bytecode=${BRIDGED_TOKENS_WRAPPER_WASM} --recall-nonce "${DRTPY_SIGN[@]}" \
