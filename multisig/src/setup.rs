...
use crate::{bridge_proxy_contract_proxy, dcdt_safe_proxy, multi_transfer_dcdt_proxy};
    ...
    + dharitri_sc_modules::pause::PauseModule
{
    ...
    #[only_owner]
    #[endpoint(pauseDcdtSafe)]
    fn pause_dcdt_safe(&self) {
        let dcdt_safe_addr = self.dcdt_safe_address().get();
    ...
    #[only_owner]
    #[endpoint(unpauseDcdtSafe)]
    fn unpause_dcdt_safe(&self) {
        let dcdt_safe_addr = self.dcdt_safe_address().get();
    ...
    #[only_owner]
    #[payable("*")]
    #[endpoint(initSupplyDcdtSafe)]
    fn init_supply_dcdt_safe(&self, token_id: TokenIdentifier, amount: BigUint) {
        let dcdt_safe_addr = self.dcdt_safe_address().get();
        ...
        self.tx()
            ...
            .single_dcdt(&payment_token, 0, &payment_amount) // enforce only single FT transfer
            .sync_call();
    }

    #[only_owner]
    #[endpoint(initSupplyMintBurnDcdtSafe)]
    fn init_supply_mint_burn_dcdt_safe(
        &self,
    ...
    #[only_owner]
    #[endpoint(dcdtSafeAddTokenToWhitelist)]
    fn dcdt_safe_add_token_to_whitelist(
        &self,
        token_id: &TokenIdentifier,
    ...
    #[only_owner]
    #[endpoint(setMultiTransferOnDcdtSafe)]
    fn set_multi_transfer_on_dcdt_safe(&self) {
        let multi_transfer_dcdt_address = self.multi_transfer_dcdt_address().get();
        let dcdt_safe_addr = self.dcdt_safe_address().get();
        self.tx()
            .to(dcdt_safe_addr)
            .typed(dcdt_safe_proxy::DcdtSafeProxy)
            .set_multi_transfer_contract_address(OptionalValue::Some(multi_transfer_dcdt_address))
            .sync_call();
    }

    #[only_owner]
    #[endpoint(setDcdtSafeOnMultiTransfer)]
    fn set_dcdt_safe_on_multi_transfer(&self) {
        let dcdt_safe_address = self.dcdt_safe_address().get();
        let multi_transfer_dcdt_addr = self.multi_transfer_dcdt_address().get();
        self.tx()
            .to(multi_transfer_dcdt_addr)
            .typed(multi_transfer_dcdt_proxy::MultiTransferDcdtProxy)
            .set_dcdt_safe_contract_address(OptionalValue::Some(dcdt_safe_address))
            .sync_call();
    }

    #[only_owner]
    #[endpoint(dcdtSafeRemoveTokenFromWhitelist)]
    fn dcdt_safe_remove_token_from_whitelist(&self, token_id: TokenIdentifier) {
        let dcdt_safe_addr = self.dcdt_safe_address().get();
        self.tx()
            .to(dcdt_safe_addr)
            .typed(dcdt_safe_proxy::DcdtSafeProxy)
    ...
    #[only_owner]
    #[endpoint(dcdtSafeSetMaxTxBatchSize)]
    fn dcdt_safe_set_max_tx_batch_size(&self, new_max_tx_batch_size: usize) {
        let dcdt_safe_addr = self.dcdt_safe_address().get();
        self.tx()
            .to(dcdt_safe_addr)
            .typed(dcdt_safe_proxy::DcdtSafeProxy)
    ...
    #[only_owner]
    #[endpoint(dcdtSafeSetMaxTxBatchBlockDuration)]
    fn dcdt_safe_set_max_tx_batch_block_duration(&self, new_max_tx_batch_block_duration: u64) {
        let dcdt_safe_addr = self.dcdt_safe_address().get();
    ...
    #[only_owner]
    #[endpoint(dcdtSafeSetMaxBridgedAmountForToken)]
    fn dcdt_safe_set_max_bridged_amount_for_token(
        &self,
        token_id: TokenIdentifier,
        max_amount: BigUint,
    ) {
    ...
    #[only_owner]
    #[endpoint(multiTransferDcdtSetMaxBridgedAmountForToken)]
    fn multi_transfer_dcdt_set_max_bridged_amount_for_token(
        &self,
        token_id: TokenIdentifier,
        max_amount: BigUint,
    ) {
    ...
    #[only_owner]
    #[endpoint(multiTransferDcdtSetMaxRefundTxBatchSize)]
    fn multi_transfer_dcdt_set_max_refund_tx_batch_size(&self, new_max_tx_batch_size: usize) {
        let multi_transfer_dcdt_addr = self.multi_transfer_dcdt_address().get();
    ...
    #[only_owner]
    #[endpoint(multiTransferDcdtSetMaxRefundTxBatchBlockDuration)]
    fn multi_transfer_dcdt_set_max_refund_tx_batch_block_duration(
        &self,
    ...
    #[only_owner]
    #[endpoint(multiTransferDcdtSetWrappingContractAddress)]
    fn multi_transfer_dcdt_set_wrapping_contract_address(
        &self,
