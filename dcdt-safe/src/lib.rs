    ...
    + dharitri_sc_modules::pause::PauseModule
{
    ...
    fn set_transaction_batch_status(
        ...
        for (tx, tx_status) in tx_batch.iter().zip(tx_statuses.to_vec().iter()) {
            // Since tokens don't exist in the DcdtSafe in the case of a refund transaction
    ...
    fn add_refund_batch(&self, refund_transactions: ManagedVec<Transaction<Self::Api>>) {
        ...
        let refund_payments = self.call_value().all_dcdt_transfers().deref().clone();
        require!(
    ...
    fn claim_refund(&self, token_id: TokenIdentifier) -> DcdtTokenPayment<Self::Api> {
        let caller = self.blockchain().get_caller();
        ...
        self.tx()
            .to(ToCaller)
            .single_dcdt(&token_id, 0, &refund_amount)
            .transfer();

        self.claim_refund_transaction_event(&token_id, caller);
        DcdtTokenPayment::new(token_id, 0, refund_amount)
    }
    ...
    fn withdraw_refund_fees_for_ethereum(
        ...
        self.tx()
            .to(multisig_owner)
            .single_dcdt(&token_id, 0, &amount_out)
            .transfer();
    ...
    fn compute_total_amounts_from_index(
        ...
        for index in start_index..end_index {
            ...
            for tx in last_batch.iter() {
                let new_payment = DcdtTokenPayment::new(tx.token_identifier, 0, tx.amount);
