...
const BRIDGE_TOKENS_WRAPPER_PATH_EXPR: &str = "drtsc:output/bridged-tokens-wrapper.drtsc.json";
// const DCDT_SAFE_CONTRACT_ADDRESS_EXPR: &str = "address:dcdt_safe";
// const DCDT_SAFE_CONTRACT_PATH_EXPR: &str = "drtsc:../dcdt-safe/output/dcdt-safe.drtsc.json";
    ...
    world.set_state_step(
        SetStateStep::new().put_account(
            ...
            Account::new()
                ...
                .dcdt_balance(UNIVERSAL_TOKEN_IDENTIFIER.to_vec(), 100_000_000u64),
        ),
    );

    world.whitebox_call(
        &bridged_tokens_wrapper,
        ScCallStep::new().from(OWNER_ADDRESS_EXPR).dcdt_transfer(
            UNIVERSAL_TOKEN_IDENTIFIER,
            0,
            100u32,
        ),
...
#[test]
#[ignore] //Ignore for now; Cannot import dcdt-safe code here
fn test_unwrap_token_create_transaction_should_fail_case_1() {
    ...
    world.whitebox_call_check(
        ...
        ScCallStep::new()
            .from(OWNER_ADDRESS_EXPR)
            .dcdt_transfer(UNIVERSAL_TOKEN_IDENTIFIER, 0, 0u32)
...
fn test_unwrap_token_create_transaction_should_fail_case_3() {
    ...
    world.whitebox_call_check(
        ...
        ScCallStep::new()
            .from(OWNER_ADDRESS_EXPR)
            .dcdt_transfer(UNIVERSAL_TOKEN_IDENTIFIER, 0, 100000u32)
            .expect(TxExpect::user_error("str:Dcdt token unavailable")),
        |sc| {
            sc.set_paused(false);
            let address = convert_to_eth_address(ETH_ADDRESS);
            sc.unwrap_token_create_transaction(
        ...
        |r| r.assert_user_error("Dcdt token unavailable"),
    );
}
...
fn test_unwrap_token_create_transaction_should_work() {
    ...
    world.whitebox_call(
        ...
        ScCallStep::new()
            .from(OWNER_ADDRESS_EXPR)
            .dcdt_transfer(UNIVERSAL_TOKEN_IDENTIFIER, 0, 100u32)
            .no_expect(),
        |sc| {
            sc.set_paused(false);
            sc.add_wrapped_token(managed_token_id!(UNIVERSAL_TOKEN_IDENTIFIER), NUM_DECIMALS);
...
fn test_wrap_tokens_should_fail_1() {
    ...
    world.set_state_step(
        SetStateStep::new().put_account(
            ...
            Account::new()
                .nonce(1)
                .balance(100_000_000u64)
                .dcdt_balance(CHAIN_TOKEN_IDENTIFIER.to_vec(), 100_000_000u64),
        ),
    );
    ...
    world.whitebox_call_check(
        ...
        ScCallStep::new()
            .from(OWNER_ADDRESS_EXPR)
            .dcdt_transfer(CHAIN_TOKEN_IDENTIFIER, 0, 100u32)
            .expect(TxExpect::user_error("str:Contract is paused")),
        |sc| {
            sc.wrap_tokens();
        },
...
fn test_unwrap_token_should_fail_case_2() {
    ...
    world.whitebox_call_check(
        ...
        ScCallStep::new()
            .from(OWNER_ADDRESS_EXPR)
            .dcdt_transfer(CHAIN_TOKEN_IDENTIFIER, 0, 0u32)
            .expect(TxExpect::user_error("str:Must pay more than 0 tokens!")),
        |sc| {
            sc.set_paused(false);
            sc.unwrap_token(managed_token_id!(CHAIN_TOKEN_IDENTIFIER));
...
fn test_unwrap_token_should_fail_case_4() {
    ...
    world.whitebox_call(
        ...
        ScCallStep::new()
            .from(OWNER_ADDRESS_EXPR)
            .dcdt_transfer(CHAIN_TOKEN_IDENTIFIER, 0, 10000u32)
            .no_expect(),
        |sc| {
            sc.set_paused(false);
    ...
    world.whitebox_call_check(
        ...
        ScCallStep::new()
            .from(OWNER_ADDRESS_EXPR)
            .dcdt_transfer(CHAIN_TOKEN_IDENTIFIER, 0, 100000u32)
            .expect(TxExpect::user_error(
                "str:Contract does not have enough funds",
            )),
        |sc| {
...
fn setup() -> ScenarioWorld {
    ...
    // let dcdt_safe_whitebox =
    //     WhiteboxContract::new(DCDT_SAFE_CONTRACT_ADDRESS_EXPR, dcdt_safe::contract_obj);
    // let dcdt_safe_code = world.code_expression(DCDT_SAFE_CONTRACT_PATH_EXPR);

    // let state_step = SetStateStep::new()
    //     .put_account(OWNER_ADDRESS_EXPR, Account::new().nonce(1))
    //     .new_address(OWNER_ADDRESS_EXPR, 1, DCDT_SAFE_ADDRESS)
    //     .block_timestamp(100);
