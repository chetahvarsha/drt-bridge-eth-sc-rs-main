...
pub trait BridgedTokensWrapper:
    dharitri_sc_modules::pause::PauseModule + events::EventsModule
{
    #[init]
    fn init(&self) {
    ...
    fn wrap_tokens(&self) -> PaymentsVec<Self::Api> {
        ...
        let original_payments = self.call_value().all_dcdt_transfers().deref().clone();
        ...
        for payment in &original_payments {
            ...
            self.send()
                .dcdt_local_mint(&universal_token_id, 0, &converted_amount);
            new_payments.push(DcdtTokenPayment::new(
    ...
    fn unwrap_token(&self, requested_token: TokenIdentifier) {
        ...
        self.tx()
            .to(ToCaller)
            .single_dcdt(&requested_token, 0, &converted_amount)
            .transfer();
    }
    ...
    fn unwrap_token_common(&self, requested_token: &TokenIdentifier) -> BigUint {
        ...
        self.send()
            .dcdt_local_burn(&universal_bridged_token_ids, 0, &payment_amount);
    ...
    fn unwrap_token_create_transaction(
        ...
        self.tx()
            ...
            .create_transaction(
                to,
                OptionalValue::Some(dcdt_safe_proxy::RefundInfo {
                    address: caller,
                    initial_batch_id: 0,
                    initial_nonce: 0,
                }),
    ...
    fn require_mint_and_burn_roles(&self, token_id: &TokenIdentifier) {
        ...
        require!(
            roles.has_role(&DcdtLocalRole::Mint) && roles.has_role(&DcdtLocalRole::Burn),
