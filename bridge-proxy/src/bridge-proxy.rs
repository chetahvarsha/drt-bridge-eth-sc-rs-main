    ...
    + dharitri_sc_modules::pause::PauseModule
{
    ...
    fn refund_transaction(&self, tx_id: usize) {
        let tx = self.get_pending_transaction_by_id(tx_id);
        let dcdt_safe_contract_address = self.dcdt_safe_contract_address().get();

        let unwrapped_token = self.unwrap_token(&tx.token_id, tx_id);
        let batch_id = self.batch_id(tx_id).get();
        self.tx()
            .to(dcdt_safe_contract_address)
    ...
    fn unwrap_token(&self, requested_token: &TokenIdentifier, tx_id: usize) -> DcdtTokenPayment {
        let payment = self.payments(tx_id).get();
        ...
        require!(
            transfers.total_rewa_amount == 0,
            "Expected only one dcdt payment"
        );
        require!(
            transfers.dcdt_payments.len() == 1,
            "Expected only one dcdt payment"
        );
        transfers.dcdt_payments.get(0)
    }
